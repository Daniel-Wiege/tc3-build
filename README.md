...
```powershell
dotnet build .\Tc3Build.slnx
dotnet run --project .\Tc3Build\Tc3Build.csproj -- build --project .\twincat\Project\Project.slnx --project-name Project --configuration Debug
```

Validate without building:

```powershell
dotnet run --project .\Tc3Build\Tc3Build.csproj -- validate --project .\twincat\Project\Project.slnx --project-name Project
```

Activate an already built configuration:

```powershell
dotnet run --project .\Tc3Build\Tc3Build.csproj -- activate --project .\twincat\Project\Project.slnx --project-name Project
```

Build and install a PLC library:

```powershell
dotnet run --project .\Tc3Build\Tc3Build.csproj -- install-library --project .\twincat\Project\Project.slnx --project-name Library
```
...
Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project --host vs2019

# Use the TwinCAT XAE Shell associated with Visual Studio 2019 only.
Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project --host xae2019

# Let Tc3Build choose: VS2026, VS2022, VS2019, XAE2022, XAE2019, XAE2017.
Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project --host auto
```
...
Tc3Build.exe install-library -p .\twincat\Project\Library\Library.tspproj -s

# Select a project by name inside a solution.
Tc3Build.exe install-library -p .\twincat\Project\Project.slnx -n Library -s

# Select the XAE project by name.
Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project -c Debug -t "TwinCAT RT (x64)"

# Explicitly build every TwinCAT project in the solution.
Tc3Build.exe build -p .\twincat\Project\Project.slnx -A
```
...
| --- | --- | --- |
| `-p`, `--project` | Solution or direct TwinCAT project path. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project` |
| `-n`, `--project-name` | Project name or relative project path inside the selected solution. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Library` |
| `-c`, `--configuration` | TwinCAT build configuration, for example `Debug` or `Release`. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project -c Debug` |
| `-t`, `--platform` | TwinCAT target platform. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project -t "TwinCAT RT (x64)"` |
| `-s`, `--silent` | Hide the IDE while keeping console logging enabled. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project -s` |
| `-H`, `--host` | Select the automation host or version. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -n Project -H vs2019` |
| `-o`, `--library-output` | Output file for `install-library`; the extension must be `.library` or `.compiled-library`. | `Tc3Build.exe install-library -p .\twincat\Project\Library\Library.tspproj -o .\artifacts\Tc3Build.library` |
| `-A`, `--all-projects` | Explicitly build every TwinCAT project in a solution. | `Tc3Build.exe build -p .\twincat\Project\Project.slnx -A` |
...
The available short forms are `-p`/`--project`, `-n`/`--project-name`, `-c`/`--configuration`, `-t`/`--platform`, `-s`/`--silent`, `-H`/`--host`, `-o`/`--library-output`, and `-A`/`--all-projects`. The `--project`/`-p` option is required for `build`, `validate`, `activate`, and `install-library`.
...
```powershell
dotnet test .\Tc3Build.slnx -c Debug
```
...
$env:TC3BUILD_INTEGRATION_PROJECT = "D:\Git\tc3-build\twincat\Project\Project.slnx"
...
dotnet test .\Tc3Build.slnx -c Debug --filter FullyQualifiedName~AutomationHostIntegrationTests
```
